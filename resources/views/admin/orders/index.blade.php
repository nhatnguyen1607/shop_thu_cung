...
<div class="mb-3">
  <form action="{{route('orders.index')}}" method="get" class="d-flex align-items-center">
    ...
    <select name="trangthai" id="filter_status" class="form-select w-auto me-2">
      ...
      <option value="Đang giao hàng">@if(request('trangthai')=='đang giao hàng') selected @endif Đang giao hàng</option>
      <option value="Giao thành công">@if(request('trangthai')=='giao thành công') selected @endif Giao thành công</option>
      <option value="Đã hủy">@if(request('trangthai')=='đã hủy') selected @endif Đã hủy</option>
    </select>
